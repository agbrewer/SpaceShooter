...
class Spawner {
  ...
  update(delta_time) {
    ...
    if (test - this.timer2 >= 1) this.spawn = true;
  }
}
...
class Player extends Body {
  ...
  update(delta_time) {
    ...
    if (this.controller.action_1 == true) {
      // bool for shooting
      let test2 = this.timer;
      if (this.shoot) {
        new Shoot(this);
        this.shoot = false;
        this.uptime = this.timer;
      }
      if (test2 - this.uptime >= 0.4) this.shoot = true;
      this.speed = 50;
    }
    super.update(delta_time);
